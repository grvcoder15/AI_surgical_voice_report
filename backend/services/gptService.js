...
// const MAX_DYNAMIC_KB_CHARS = 30000;
...
//     const dynamicKnowledgeBase = await getDynamicKnowledgeBaseText();
//     const prompt = buildSurgicalReportPrompt(transcript, dynamicKnowledgeBase);
...
// async function getDynamicKnowledgeBaseText() {
//   try {
...
//     const trimmedContent = String(latestKB.content).trim();
//     if (!trimmedContent) {
//       return '';
//     }

//     if (trimmedContent.length > MAX_DYNAMIC_KB_CHARS) {
//       logger.warn(
//         `GPT Service: Latest KB content exceeded ${MAX_DYNAMIC_KB_CHARS} chars and was truncated for prompt safety`
//       );
//       return trimmedContent.slice(0, MAX_DYNAMIC_KB_CHARS);
//     }

//     logger.info(`GPT Service: Loaded dynamic KB version ${latestKB.version} from PostgreSQL`);
//     return trimmedContent;
//   } catch (error) {
//     logger.warn(`GPT Service: Unable to load dynamic KB from database: ${error.message}`);
//     return '';
//   }
// }
...
//  */
// function buildSurgicalReportPrompt(transcript, dynamicKnowledgeBase = '') {
//   const dynamicKnowledgeBaseSection = dynamicKnowledgeBase
//     ? `
// ═══════════════════════════════════════════════════════════
// CURRENT DYNAMIC KNOWLEDGE BASE (LATEST UPLOAD)
// ═══════════════════════════════════════════════════════════

// Use this uploaded KB as the current surgeon-specific reference material.
// Treat it as dynamic guidance that supplements the static library below.
// Never copy it verbatim into the final report.
...
// No uploaded dynamic KB is currently available from PostgreSQL.
// `;
...
// Priority: 1) PostgreSQL DB (if configured later)
//           2) config/knowledge_base.txt (current default)
//
// NOTE: No changes needed in this file when KB is updated.
// Dr. Oren uploads new KB via web page → knowledge_base.txt updates automatically
// → next report generation picks it up instantly.
// ─────────────────────────────────────────────────────────────────────────────

async function loadKnowledgeBase() {
  // DB support — will activate automatically when DATABASE_URL is added to .env
  if (process.env.DATABASE_URL) {
    try {
      const db = require('../config/db');
      const result = await db.query(
        'SELECT content FROM knowledge_base ORDER BY version DESC LIMIT 1'
      );
      if (result.rows.length > 0) {
        logger.info('KB loaded from PostgreSQL database');
        return result.rows[0].content;
      }
      logger.warn('KB table is empty in DB — falling back to file');
    } catch (err) {
      logger.warn(`DB KB load failed: ${err.message} — falling back to file`);
    }
  }

  // Default: load from config/knowledge_base.txt
  try {
    const kbPath = path.join(__dirname, '..', 'config', 'knowledge_base.txt');
    const content = fs.readFileSync(kbPath, 'utf8');
    if (content.trim()) {
      logger.info('KB loaded from config/knowledge_base.txt');
      return content;
    }
    logger.warn('knowledge_base.txt is empty — using empty KB');
    return '';
  } catch (err) {
    logger.error(`KB file load failed: ${err.message}`);
    return '';
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// STATIC PROMPT PARTS
// These never change — only KB section changes when Dr. Oren uploads new KB
// ─────────────────────────────────────────────────────────────────────────────
...
8. INDICATION FOR SURGERY
─────────────────────────────────────────────────────────
[CRITICAL — Write a SEPARATE justification paragraph for EACH
procedure performed in this case.

Format:
  [Procedure 1 name]:
  [Paragraph — why THIS procedure was medically necessary,
  risk to function/life if untreated, specific to THIS case]

  [Procedure 2 name]:
  [Separate paragraph...]

Each paragraph: minimum 2-3 sentences, case-specific, persuasive.
AI generates all. Doctor never asked.]
...
INCLUDE IN ORDER:
  ...
  b) Tourniquet (type, pressure if limb only, duration, continuous/reperfusion)
  c) Wound inspection and debridement if applicable
     (size l x w cm, sq cm ONLY if both dimensions given, tissue layers,
...
RULES:
  - NEVER "in a similar fashion" or "as described above"
  - Each structure fully and independently
  - KB key elements in fresh language
  - Elaborate on anatomy encountered]

─────────────────────────────────────────────────────────
10. QUALITY NOTES (INTERNAL — NOT PART OF SIGNED REPORT)
─────────────────────────────────────────────────────────
[Internal review only. List:
  - Unclear/misheard terms (label "unclear — verify before signing")
  - Values corrected during confirmation
  - Ambiguous anatomical/instrument names
  - Required fields not provided
If none: "No quality concerns identified."]
...
async function generateWithPerplexity(transcript) {
  try {
    ...
    const prompt = await buildSurgicalReportPrompt(transcript);
    ...
    logger.info(`Perplexity: Sending request using model ${model}...`);
    logger.info('='.repeat(80));
    logger.info('PERPLEXITY PROMPT (FULL)');
    logger.info('='.repeat(80));
    console.log(prompt);
    ...
    if (!generatedReport) {
      logger.error(`Perplexity: Unexpected response: ${JSON.stringify(response.data)}`);
      throw new Error('Invalid response from Perplexity API');
    }

    logger.info('✅ Perplexity: Successfully generated report');
    logger.info(`Report length: ${generatedReport.length} characters`);
    return generatedReport;
  } catch (error) {
    logger.error(`Perplexity API Error: ${error.message}`);
    if (error.response) {
...
// ─────────────────────────────────────────────────────────────────────────────
// PROMPT BUILDER
// Combines: STATIC_RULES + transcript + dynamic KB + STATIC_FORMAT
// KB is loaded fresh on every call — always uses latest version
// ─────────────────────────────────────────────────────────────────────────────

async function buildSurgicalReportPrompt(transcript) {
...
[No knowledge base currently loaded. Apply only what is
explicitly stated in the transcript. Do not assume any
standard techniques.]
`;
